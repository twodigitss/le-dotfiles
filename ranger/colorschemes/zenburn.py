...
class zenburn(ColorScheme):
    progress_bar_color = 108; #COLOR DE LA BARRA DE PROGRESO (RECOMIENDO NEGRO 232)
    ...
    def use(self, context):
        ...
        #-----------COLORES NORMALES BASICOS----------
            ...
            if context.empty or context.error:
                fg = 174;   #FOLDER VACIO (letras rojas "empty")
                bg = 235            
            if context.border:
                fg = 248;   #SEPARADOR (bordes blancos entre carpetas)
            if context.image:
                fg = 109;   #IMAGENES (png,jpeg,jpg,raw,etc)
            if context.video:
                fg = 66;   #VIDEOS (mp4,mov,mkv,etc)
            if context.audio:
                fg = 116;   #AUDIOS (mp3,ogg,etc)
            if context.document:
                fg = 151;   #ARCHIVOS (txt,docx,tar,config,etc)
            if context.container:
                attr |= bold
                fg = 174;   #COMPRIMIDOS (zip,rar,tgzip,etc)
            if context.directory:
                attr |= bold
                fg = 223;   #CARPETAS (folders)
                attr |= bold
            elif context.executable and not \
                    any((context.media, context.container,
                         context.fifo, context.socket)):
                attr |= bold
                fg = 108;   #EJECUTABLES (exe,bat,sh,msdos,etc)
            ...
            if context.link:
                fg = 223 if context.good else 116; #CARPETAS/ARCHIVOS ENLAZADOS
                bg = 234
            if context.bad:
                bg = 235
        ...
        #-------------CONTEXTO SEELECCIONADO-----------
            if not context.selected and (context.cut or context.copied):
                fg = 108;   #COLOR AL CORTAR/COPIAR CARPETAS-ARCHIVOS
            ...
            if context.main_column:
                ...
                if context.marked:
                    attr |= bold
                    fg = 223; #SELECCIONAR CARPETAS (CON TECLA ESPACIO)
            if context.badinfo:
                if attr & reverse:
        ...
        #-------------TITLEBAR---------
            ...
            if context.hostname:
                attr |= bold
                fg = 174 if context.bad else 180 #COLOR DEL USUARIO+HOST
            elif context.directory:
                fg = 223; #COLOR DE LA RUTA DEL DIRECTORIO ACTUAL (EN LA TITLEBAR)
            elif context.tab:
                if context.good:
                    bg = 180; #COLOR DE LAS TABS (AL PRESIONAR CTRL+N)
            #folders linkeados
            elif context.link:
        ...
        #------------BARRA DE ESTATUS-----
            ...
            if context.permissions:
                if context.good:
                    fg = 108; #PERMISOS EN LA TITLEBAR
                elif context.bad:
                    fg = 174; #PERMISOS EN LA TITLEBAR (POR SI ACASO)
            if context.marked:
                attr |= bold | reverse
