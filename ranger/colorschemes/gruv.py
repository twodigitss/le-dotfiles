...
class vscode(ColorScheme):
    ...
    def use(self, context):
        ...
        #-----------COLORES NORMALES BASICOS----------
            ...
            if context.video:
                fg = 138;   #VIDEOS (mp4,mov,mkv,etc)
            if context.audio:
                fg = 65;   #AUDIOS (mp3,ogg,etc)
            if context.document:
                fg = 189;   #ARCHIVOS (txt,docx,tar,config,etc)
            if context.container:
                fg = 223;   #COMPRIMIDOS (zip,rar,tgzip,etc)
            if context.directory:
                fg = 108;   #CARPETAS (folders) 
                attr |= bold
            elif context.executable and not \
                    any((context.media, context.container,
                         context.fifo, context.socket)):
                fg = 150;   #EJECUTABLES (exe,bat,sh,msdos,etc)
            ...
            if context.link:
                fg = 223 if context.good else 224; #CARPETAS/ARCHIVOS ENLAZADOS
        ...
        #-------------CONTEXTO SEELECCIONADO-----------
            ...
            if context.main_column:
                ...
                if context.marked:
                    attr |= normal
                    fg = 66; #SELECCIONAR CARPETAS (CON TECLA ESPACIO)
            if context.badinfo:
                if attr & reverse:
        ...
        #-------------TITLEBAR---------
            ...
            if context.hostname:
                attr |= bold
                fg = 138 if context.bad else 138 #COLOR DEL USUARIO+HOST
            elif context.directory:
                fg = 254; #COLOR DE LA RUTA DEL DIRECTORIO ACTUAL (EN LA TITLEBAR)
            elif context.tab:
                if context.good:
                    fg = 232; #COLOR DE LAS TABS (AL PRESIONAR CTRL+N)
                    bg = 222; #COLOR DE LAS TABS (AL PRESIONAR CTRL+N)
            #folders linkeados
            elif context.link:
                fg = 65; #COLOR DE FOLDERS LINKEADOS EN EL DIR ACTUAL (TITLEBAR)
