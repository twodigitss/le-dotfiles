...
class kiwi(ColorScheme):
    ...
    def use(self, context):
        ...
        #-----------COLORES NORMALES BASICOS----------
            ...
            if context.image:
                fg = 223;   #IMAGENES (png,jpeg,jpg,raw,etc)
            if context.video:
                fg = 189;   #VIDEOS (mp4,mov,mkv,etc)
            if context.audio:
                fg = 150;   #AUDIOS (mp3,ogg,etc)
            if context.document:
                fg = 225;   #ARCHIVOS (txt,docx,tar,config,etc)
            if context.container:
                fg = 230;   #COMPRIMIDOS (zip,rar,tgzip,etc)
            if context.directory:
                fg = 254;   #CARPETAS (folders)
                attr |= bold
            elif context.executable and not \
                    any((context.media, context.container,
                         context.fifo, context.socket)):
                fg = 210;   #EJECUTABLES (exe,bat,sh,msdos,etc) 
            ...
            if context.link:
                fg = 230 if context.good else 225; #CARPETAS/ARCHIVOS ENLAZADOS
        ...
        #-------------CONTEXTO SEELECCIONADO-----------
            if not context.selected and (context.cut or context.copied):
                fg = 223;   #COLOR AL CORTAR/COPIAR CARPETAS-ARCHIVOS
        ...
        #-------------TITLEBAR---------
            ...
            if context.hostname:
                attr |= bold
                fg = 210 if context.bad else 210 #COLOR DEL USUARIO+HOST
            elif context.directory:
                fg = 231; #COLOR DE LA RUTA DEL DIRECTORIO ACTUAL (EN LA TITLEBAR)
            elif context.tab:
                if context.good:
                    bg = 225; #COLOR DE LAS TABS (AL PRESIONAR CTRL+N)
                    fg = 232;
            #folders linkeados
            elif context.link:
                fg = 230; #COLOR DE FOLDERS LINKEADOS EN EL DIR ACTUAL (TITLEBAR)
        ...
        #------------BARRA DE ESTATUS-----
            ...
            if context.permissions:
                if context.good:
                    fg = 230; #PERMISOS EN LA TITLEBAR
                elif context.bad:
                    fg = 230; #PERMISOS EN LA TITLEBAR (POR SI ACASO)
            if context.marked:
                attr |= bold | reverse
                fg = 230; #LETRAS <MRK> AL MARCAR UN ARCHIVO CON ESPACIO
            if context.message:
                if context.bad:
                    attr |= bold
                    fg = 174
