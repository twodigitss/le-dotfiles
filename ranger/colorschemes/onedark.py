...
class vscode(ColorScheme):
    ...
    def use(self, context):
        ...
        #-----------COLORES NORMALES BASICOS----------
            ...
            if context.image:
                fg = 193;   #IMAGENES (png,jpeg,jpg,raw,etc)
            if context.video:
                fg = 189;   #VIDEOS (mp4,mov,mkv,etc)
            if context.audio:
                fg = 117;   #AUDIOS (mp3,ogg,etc)
            if context.document:
                fg = 153;   #ARCHIVOS (txt,docx,tar,config,etc)
            if context.container:
                fg = 217;   #COMPRIMIDOS (zip,rar,tgzip,etc)
            if context.directory:
                fg = 183;   #CARPETAS (folders) 
                #fg = 252;   #CARPETAS (folders) 
                attr |= bold
            elif context.executable and not \
                    any((context.media, context.container,
                         context.fifo, context.socket)):
                fg = 229;   #EJECUTABLES (exe,bat,sh,msdos,etc)
            ...
            if context.link:
                fg = 217 if context.good else 156; #CARPETAS/ARCHIVOS ENLAZADOS
        ...
        #-------------CONTEXTO SEELECCIONADO-----------
            if not context.selected and (context.cut or context.copied):
                fg = 240;   #COLOR AL CORTAR/COPIAR CARPETAS-ARCHIVOS
            ...
            if context.main_column:
                ...
                if context.marked:
                    attr |= normal
                    fg = 147; #SELECCIONAR CARPETAS (CON TECLA ESPACIO)
            if context.badinfo:
                if attr & reverse:
        ...
        #-------------TITLEBAR---------
            ...
            if context.hostname:
                attr |= bold
                fg = 229 if context.bad else 229 #COLOR DEL USUARIO+HOST
            ...
            #folders linkeados
                ...
                fg = 217; #COLOR DE FOLDERS LINKEADOS EN EL DIR ACTUAL (TITLEBAR)
        ...
        #------------BARRA DE ESTATUS-----
            ...
            if context.permissions:
                if context.good:
                    fg = 193; #PERMISOS EN LA TITLEBAR
                elif context.bad:
                    fg = 193; #PERMISOS EN LA TITLEBAR (POR SI ACASO)
            if context.marked:
                attr |= bold | reverse
                fg = 153; #LETRAS <MRK> AL MARCAR UN ARCHIVO CON ESPACIO
            if context.message:
                if context.bad:
                    attr |= bold
                    fg = 174
